 ...
 *
 * Extends TrueSightProgramShell.renderEventListItem to render oracle-specific
 * payload fields (hexagrams, advisory_summary, qmdj_card, mood) inside the
 * expandable <details> block on credential pages.
 *
...
(function () {
  ...
  var _origRender = window.TrueSightProgramShell && window.TrueSightProgramShell._origRenderEventListItem;

  // Store original if not already wrapped
  if (!_origRender && window.TrueSightProgramShell) {
    // We can't easily monkey-patch the closure, so instead we hook into
    // the credential rendering by patching the renderCredential function's
    // call to renderEventListItem. Since it's in a closure, we need a
    // different approach.
  }

  // Alternative: expose a helper that the credential page can call to
  // render oracle-specific event detail blocks.
