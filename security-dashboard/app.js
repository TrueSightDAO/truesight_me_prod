...
// Reads security-dashboard.json from treasury-cache and renders the view
...
function daysColor(d) {
  if (d === null || d === undefined) return "#666";
...
function renderScore(data) {
  ...
  const color = daysColor(n >= 75 ? 90 : n >= 50 ? 30 : 0);
...
function renderWeb(data) {
  ...
  sites.forEach(site => {
    ...
    const headerTotal = (headers.present || []).length + (headers.missing || []).length;
...
function renderPhishing(data) {
  ...
  container.innerHTML = `
    <div class="sd-card">
      ...
      <div class="sd-big-num">${s.blacklisted_domains || 0}</div>
    </div>
    <div class="sd-card">
      <h3>Verified Domains</h3>
      <div class="sd-big-num">${s.verified_domains || 0}</div>
    </div>
    <div class="sd-card">
      <h3>Blacklisted People</h3>
      <div class="sd-big-num">${s.blacklisted_people || 0}</div>
    </div>
    <div class="sd-card">
      <h3>Blacklisted URLs</h3>
      <div class="sd-big-num">${s.blacklisted_urls || 0}</div>
    </div>
    <div class="sd-card sd-card-wide">
